...
import { MdArrowBack, MdAssignment, MdSearch } from "react-icons/md";
...
export default function TrainerTasks() {
    ...
    const fetchTodos = async () => {
        try {
            ...
            // Filter todos by trainer
            const allTodos = res.data || [];
            const trainerTodos = allTodos.filter(
                t => t.todo?.trainer?.id == trainerId || t.todo?.trainerId == trainerId
            );
            setTodos(trainerTodos);
            
            // Get trainer info from first todo
            if (trainerTodos.length > 0) {
                setTrainerInfo(trainerTodos[0].todo?.trainer);
            }
        } catch (err) {
    ...
    return (
        <div className="p-6">
            ...
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 className="text-2xl font-bold">Tasks from {trainerInfo?.user?.username || "Trainer"}</h1>
                    <p className="text-gray-600">{trainerInfo?.specialization || ""}</p>
                </div>
            </div>
            ...
            {filteredTodos.length === 0 ? (
                <div className="bg-white rounded-lg shadow p-12 text-center text-gray-400">
                    ...
                    <p>No tasks from this trainer</p>
                </div>
            ) : (
